...
export default function GuidedPour({ recipe, onFinish, onExit }: Props) {
  ...
  const tone = current > target + TOLERANCE ? 'over' : withinRange && current > 0 ? 'ok' : 'active';
  ...
  // ── Beep at the start of each pour step ──────────────────────────────────
  useEffect(() => {
    beep('step');
  }, [stepIndex]);
  ...
  // ── Auto progression: rest elapsed → next pour ───────────────────────────
  useEffect(() => {
  ...
  return (
    <div className={styles.screen}>
      <header className={styles.header}>
        <button className={styles.iconBtn} onClick={onExit} aria-label="Back">
        ...
        <div className={styles.title}>
          ...
          <p>
            ...
            {recipe.grindSize && ` • ${recipe.grindSize.replace('-', ' ')} grind`}
          </p>
        </div>
        <button className={styles.iconBtn} aria-label="Options">
          <IconDots size={22} />
        </button>
      </header>

      <nav className={styles.steps} aria-label="Pour steps">
        {recipe.steps.map((s, i) => {
          const state = i < stepIndex ? 'done' : i === stepIndex ? 'active' : 'todo';
          return (
            <div key={s.index} className={styles.stepItem}>
              <div className={`${styles.stepDot} ${styles[`stepDot_${state}`]}`}>
                {state === 'done' ? <IconCheck size={13} /> : i + 1}
              </div>
              <span className={`${styles.stepName} ${state === 'active' ? styles.stepNameActive : ''}`}>
                {s.label}
              </span>
              {i < recipe.steps.length - 1 && (
                <div className={`${styles.stepConn} ${i < stepIndex ? styles.stepConnDone : ''}`} />
              )}
            </div>
          );
        })}
        <div className={styles.stepItem}>
          <div className={`${styles.stepDot} ${phase === 'done' ? styles.stepDot_done : styles.stepDot_todo}`}>
            <IconFlag size={12} />
          </div>
      ...
      <div className={styles.body}>
        {(() => {
          ...
          return (
            <div key={s.index} className={styles.activeRow}>
              <div className={styles.rowBody}>
                <div className={styles.stepHead}>
                  <div>
                    <h2 className={styles.stepTitle}>{step.label}</h2>
                    <p className={styles.stepSub}>
                      Up to {step.target} g • Wait until {formatTime(step.waitUntil)}
                    </p>
                  </div>
                ...
                <WaterGauge
                  ...
                  marks={recipe.steps.map(s => s.target)}
                  flow={flow}
                  tone={tone}
                />

                <section className={styles.card}>
                  <span className={styles.cardLabel}>THIS STEP'S TARGET</span>
                  <div className={styles.rangeHead}>
                    ...
                    <div className={styles.rangeItemRight}>
                      ...
                      <span className={styles.rangeReal}>{Math.round(current)} g</span>
                    </div>
                    <div className={`${styles.badge} ${withinRange ? styles.badgeOk : styles.badgeOut}`}>
                      <span className={styles.badgeMain}>
                        {withinRange ? <IconCheck size={14} /> : <IconAlert size={14} />}
                        {over > 0 ? `+${Math.round(over)} g` : withinRange ? 'In range' : 'Out'}
                      </span>
                      <span className={styles.badgeSub}>±{TOLERANCE} g</span>
                    </div>
                  </div>
                </section>
                ...
                <section className={styles.card}>
                  ...
                  <AccumulationChart recipe={recipe} history={history} elapsed={elapsed} current={current} />
                </section>
                  ...
                  <div className={styles.pourToRow}>
                    <div className={styles.pourToMain}>
                      ...
                      <span className={styles.pourToDelta}>
                        Target {target} g
                        {remaining > 0 && (
                          <span className={styles.pourToGo}> • {Math.round(remaining)} g to go</span>
                        )}
                      </span>
                    </div>
                    ...
                    <div className={styles.adjust}>
                      <button
                        ...
                        onClick={() => setCurrent(c => Math.max(0, c - 5))}
                  ...
                  <div className={styles.nextRow}>
                    <div>
                      ...
                      <div className={styles.nextAction}>
                        <IconTimer size={16} className={styles.actionIcon} />
                        {phase === 'resting' ? (
                          <span>Next pour in <strong>{formatTime(Math.ceil(rest))}</strong></span>
                        ) : (
                          <span>Wait until <strong>{formatTime(step.waitUntil)}</strong></span>
                        )}
                      </div>
                    </div>
                    <button className={styles.skipBtn} onClick={advanceStep}>
      ...
      <footer className={styles.footer}>
        <button className={styles.footBtn} onClick={() => setRunning(r => !r)} disabled={phase === 'done'}>
          {running ? <IconPause size={16} /> : <IconPlay size={16} />}
          {running ? 'Pause' : 'Resume'}
        </button>
        <button className={styles.footBtn} onClick={handleReset} disabled={phase === 'done'}>
          <IconScale size={16} />
          Tare
        </button>
        <button className={`${styles.footBtn} ${styles.finish}`} onClick={handleFinish}>
